-- Ensure all tables exist for proper kill switch functionality
...
-- Update redemption_history to have proper foreign keys if needed
DO $$ 
BEGIN
    -- Check if business_id column exists in redemption_history
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'redemption_history' AND column_name = 'business_id'
    ) THEN
        ALTER TABLE redemption_history ADD COLUMN business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;
        ...
        SET business_id = (
            ...
            FROM punchcards p 
            WHERE p.id = redemption_history.punchcard_id
        )
        WHERE business_id IS NULL;
    END IF;
END $$;

-- Add indexes for better performance during deletion
